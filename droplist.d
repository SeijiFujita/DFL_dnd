...
//			liststring = ea.data.getData(DataFormats.utf8).getString(); // Get the dropped data.
			liststring = ea.data.getText() ; // Get the dropped data.
...
//			items.insert(i, liststring); // Insert it!
			items.insert(i, dfl.internal.utf.fromAnsi(cast(immutable(char)*)liststring, liststring.length)); // Insert it!
