...
++/
// outLog1();
void outLog1(string file = __FILE__, int line = __LINE__, T)(T t)
//void outLog1(string file = __FILE__, int line = __LINE__, string T)(T t)
{
version (outDebugLog) 
	_outDebugLog(format("%s:%d:", file, line), t);
}
// outLog();
...
void setDebugLog(int flag = 1)
{
	...
	string logfilename = Runtime.args[0];
...
static void _outLogV(...)
{
	...
	for (int i = 0; i < _arguments.length; i++)
	{
		if (_arguments[i] == typeid(bool)) {
			...
			assert(0, "debuglog.d: Unknown type");
		}
	}
	_outLoglf(str);
}
