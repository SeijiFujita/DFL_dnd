...
// http://msdn.microsoft.com/ja-jp/library/system.windows.forms.drageventargs%28v=vs.110%29.aspx

class DragEventArgs: EventArgs
{
	this(IDataObject dataObject, int keyState, int x, int y,
		DragDropEffects allowedEffect, DragDropEffects effect)
	{
		_data = new DataObject(dataObject);
		_keyState = keyState;
		_x = x;
		_y = y;
...
//	final @property IDataObject data() // getter
	final @property DataObject data() // getter
	{
		return _data;
	}
...
@property void allowDrop(bool byes) // setter
{
		...
		if (!droptarget)
		{
			...
			if(isHandleCreated)
			{
				...
				case DRAGDROP_E_INVALIDHWND:
					throw new DflException("RegisterDragDrop:DRAGDROP_E_INVALIDHWND");
		        case E_OUTOFMEMORY:
					throw new DflException("RegisterDragDrop:E_OUTOFMEMORY");
				default:
					droptarget = null;
	...
	else
	{
        ...
        case DRAGDROP_E_NOTREGISTERED:
			throw new DflException("RevokeDragDrop:DRAGDROP_E_NOTREGISTERED");
		case DRAGDROP_E_INVALIDHWND:
			throw new DflException("RevokeDragDrop:DRAGDROP_E_INVALIDHWND");
        case E_OUTOFMEMORY:
			throw new DflException("RevokeDragDrop:E_OUTOFMEMORY");
		default:
			droptarget = null;
			throw new DflException("Unable to unregister drag-drop");
		}
	}
}
...
private static class DropTarget: xComObject, IDropTarget
{
	this(Control rctrl)
	{
		this.ctrl = rctrl;
		outLog("#DropTarget:Start");
	}
	~this()
	{
		outLog("#DropTarget:End");
	}
//@----------------------------------------------------------------------------
	extern(Windows)
	override HRESULT QueryInterface(IID* riid, void** ppv)
	{
		outLog("DropTarget:QueryInterface");
...
/++
 DragEnter
 ドラッグドロップ操作
 1.ドラッグされるデータを受け入れるか否かを選択します。
 2.他のアプリケーション(自アプリも)からドラッグ状態で DragEnter は呼び出されます。
 3.ドロップデータをチェックしドロップ動作するかを(COPY/MOVE/NONE)を決定します。
 IDataObject pDataObject: ドロップデータ
 DWORD grfKeyState: キー、マウスの状態
 POINTL pt: マウスの位置
 DWORD *pdwEffect: DROPEFFECT ドロップ動作を指定
...
++/
	...
	HRESULT DragEnter(IDataObject pDataObject, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
	{
		...
		try
		{
			...
			scope DragEventArgs ea = 
				new DragEventArgs(pDataObject, cast(int)grfKeyState, pt.x, pt.y,
					cast(DragDropEffects)*pdwEffect, DragDropEffects.NONE);
			
			ctrl.onDragEnter(ea);
			*pdwEffect = ea.effect;
			outLog("DropTarget:DragEnter:end");
		}
		catch(DThrowable e)
		{
...
/++
 DragOver
 1.ドロップ状態でマウスのポインタが移動すると呼ばれる POINTL に注意
...
//			outLog("DropTarget:DragOver");
			assert(_DataObject !is null);

			scope DragEventArgs ea = 
				new DragEventArgs(_DataObject, cast(int)grfKeyState, pt.x, pt.y, 
					cast(DragDropEffects)*pdwEffect, DragDropEffects.NONE);
...
/++
 DragLeave
 ドロップ状態でマウスのポインタがcontol(Window)の外に出ると呼ばれる
...
++/
	...
	private DWORD DropEffect(DWORD grfKeyState, POINTL pt, DWORD dwAllowed)
	{
		...
		{
			dwEffect = dwAllowed & DragDropEffects.COPY;
		}
		else if (grfKeyState & MK_SHIFT)
		{
			dwEffect = dwAllowed & DragDropEffects.MOVE;
		}
		...
		if (dwEffect == 0)
		{
			if (dwAllowed & DragDropEffects.COPY)
				dwEffect = DragDropEffects.COPY;
			
			if (dwAllowed & DragDropEffects.MOVE)
				dwEffect = DragDropEffects.MOVE;
		}
		return dwEffect;
	}
private:
...
private static class DropSource: xComObject, IDropSource
{
	this(Control ctrl)
	{
		this.ctrl = ctrl;
		mbtns = Control.mouseButtons;
		outLog("#DropSource:Start");
	}
	~this()
	{
		outLog("#DropSource:End");
	}
...
//@----------------------------------------------------------------------------
	...
	override HRESULT QueryInterface(IID* riid, void** ppv)
	{
		outLog("DropSource:QueryInterface");
	...
	DragAction scanMouseKeys(BOOL fEscapePressed, DWORD grfKeyState)
	{
		DragAction act;
...
++/
	...
	HRESULT QueryContinueDrag(BOOL fEscapePressed, DWORD grfKeyState)
	{
		...
		try
		{
			DragAction act = scanMouseKeys(fEscapePressed, grfKeyState);
			
			scope QueryContinueDragEventArgs ea = 
				new QueryContinueDragEventArgs(cast(int)grfKeyState, fEscapePressed != FALSE, act);
...
++/
	...
	HRESULT GiveFeedback(DWORD dwEffect)
	{
		...
		try
		{
			scope GiveFeedbackEventArgs ea = 
				new GiveFeedbackEventArgs(cast(DragDropEffects)dwEffect, true);
...
final DragDropEffects doDragDrop(DataObject data, DragDropEffects allowedEffects)
{
	outLog("#doDragDrop:start");
	DWORD effect;
	DropSource dropsrc = new DropSource(this);
	//
	switch(DoDragDrop(data.getDataObject, dropsrc, cast(DWORD)allowedEffects, &effect))
	{
	case DRAGDROP_S_DROP: // All good.
		outLog("doDragDrop:DRAGDROP_S_DROP");
		break;
	
	case DRAGDROP_S_CANCEL:
		outLog("doDragDrop:DRAGDROP_S_CANCEL");
	...
	outLog("#doDragDrop:end");
	delete dropsrc;
	return cast(DragDropEffects)effect;
}

final DragDropEffects doDragDrop(string text, DragDropEffects allowedEffects)
{
	auto data = new DataObject;
	data.setText(text);
	auto result = doDragDrop(data, allowedEffects);
	delete data;
	return result;
}
...
{
	outLog1("doDragDrop(Data obj, DragDropEffects allowedEffects)");
